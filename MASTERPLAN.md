...
#### 8-13. 静的サイト生成 (SSG) / 動的・静的切り替えモード
```
課題:
- 動的サーバーを公開せずにコンテンツを配信したい (セキュリティ・コスト)
- Jamstack / CDN配信との親和性
- WordPress には標準の SSG 機能がなく、プラグイン依存

実装内容:
- rustpress-ssg クレート新設
- CLIコマンド: rustpress generate --output ./dist --base-url https://example.com
  → 全投稿・固定ページ・カテゴリ/タグアーカイブ・RSS/Atomフィードを HTML/XML に書き出し
  → /wp-admin, /wp-json は含まない (静的ファイルのみ)
  → 画像・CSS・JS をdistディレクトリにコピー
  → インクリメンタルビルド: 変更されたページのみ再生成 (content hash比較)
  → 並列レンダリング: Tokio タスクで並列処理 (投稿数 ÷ コア数)
  → ビルドレポート: 生成ファイル数・所要時間・スキップ数を表示

動的・静的切り替えモード:
  [動的モード] (デフォルト)
    → Axumサーバーがリクエストをリアルタイム処理
    → コメント・ログイン・フォームなど全機能利用可能
    → 管理画面 /wp-admin はローカル/VPN のみで稼働

  [静的モード] (SSG)
    → rustpress generate で dist/ にビルド
    → dist/ を Nginx / S3 / Cloudflare Pages 等で配信
    → 管理画面はローカルで起動し、内容を変更 → 再ビルド → デプロイ
    → 公開サーバーはRustバイナリ不要 (純粋な静的ファイル配信)

  [ハイブリッドモード] (将来)
    → 静的ページ + 動的エンドポイント (/wp-json/*, /comments/*)
    → Edge Functions / Lambda@Edge との組み合わせ

CLIサブコマンド統合 (8-4との連携):
  rustpress generate               # 全ページ生成
  rustpress generate --watch       # ファイル変更を監視して自動再生成
  rustpress generate --clean       # dist/ を一度クリアしてフルビルド
  rustpress generate --dry-run     # 生成対象のリストのみ表示
  rustpress serve --static dist/   # dist/ をAxumで簡易ホスティング

セキュリティ上の利点 (WordPress比較):
  → 公開サーバーにDBが不要 → SQLi攻撃面ゼロ
  → PHPランタイム不要 → PHP RCE脆弱性の排除
  → /wp-admin を公開ネットワークから完全隔離
  → 攻撃対象面 (attack surface) が静的ファイルサーバーのみ

パフォーマンス上の利点:
  → TTFB < 10ms (CDNキャッシュ or Nginx sendfile)
  → Rustビルドは CPUバウンド → 1000投稿を数秒で生成見込み
  → WordPress SSGプラグイン (WP2Static等) 比で数十倍高速化を目標
```
