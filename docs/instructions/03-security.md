...
## Phase 2: ホスト型サービスのセキュリティ要件

RustPressの変換エンジン（php-to-rust等）は将来ホスト型Webサービスとして公開する。
その際に必要となるセキュリティ要件を以下に定義する。

**現時点ではCLIツールのため即時対応不要。** ホスト型Web UI構築時に着手する。
...
#### 認証・認可
- [ ] ユーザー登録・ログイン（OAuth2 / メール+パスワード）
- [ ] Stripe顧客IDとの紐付け
- [ ] APIキー発行・管理（ユーザーごと）
- [ ] ロールベース: free / pro / enterprise

#### マルチテナント分離
- [ ] ユーザーAの変換ジョブ・結果にユーザーBがアクセス不可
- [ ] 変換ジョブの実行環境をサンドボックス化（コンテナ分離）
- [ ] アップロードされたソースコードの暗号化保存
- [ ] ジョブ完了後のソースコード自動削除（保持期間設定可能）
...
#### 入力バリデーション
- [ ] アップロードファイルサイズ制限（無料: 1MB、Pro: 10MB）
- [ ] 悪意あるソースコード検出（フォーク爆弾、無限ループ等）
- [ ] 変換対象の言語・ファイル形式の制限
- [ ] cargo check実行のタイムアウト設定

#### LLM APIキー管理
- [ ] サービス側のAPIキーをVault等で安全に管理
- [ ] キーローテーションの自動化
- [ ] APIキーがログ・エラーメッセージに漏洩しないこと
- [ ] ユーザーのBYOキーを安全に保存（暗号化、使用時のみ復号）
